 ...
 *      Copyright (C) 2018 Giuliano Belinassi
 *
 *
...
{
    GRegex* regex_connected;
    GRegex* regex_disconnected;
    GMatchInfo* match;
    int status;
    char* output = NULL;
    char* ori_locale;
...
//([0-9]+x[0-9]+.[0-9]+.[0-9]+)
    regex_connected    = g_regex_new( "\n([-a-zA-Z]+[-0-9]*) +connected ([^(\n ]*)[^\n]*"
                         "((\n +[0-9]+x[0-9]+[^\n]+)+)",
                         0, 0, NULL );
    regex_disconnected = g_regex_new( "\n([-a-zA-Z]+[-0-9]*) +disconnected +[0-9]+x[0-9]+.*",
                         0, 0, NULL );

    if( g_regex_match( regex_connected, output, 0, &match ) )
    {
        do {
    ...
    if (g_regex_match( regex_disconnected, output, 0, &match ))
    {
        do {
            Monitor* m = g_new0( Monitor, 1 );
    ...
    g_regex_unref( regex_connected );
    g_regex_unref( regex_disconnected );
...
//    gtk_about_dialog_set_url_hook( open_url, NULL, NULL);
    ...
    gtk_about_dialog_set_program_name ( (GtkAboutDialog*)about_dlg, _( "ULXRandR" ) );
    //gtk_about_dialog_set_logo( (GtkAboutDialog*)about_dlg, gdk_pixbuf_new_from_file(  PACKAGE_DATA_DIR"/pixmaps/lxrandr.png", NULL ) );
    gtk_about_dialog_set_logo_icon_name( (GtkAboutDialog*)about_dlg, "video-display" );
    gtk_about_dialog_set_copyright ( (GtkAboutDialog*)about_dlg, _( "Copyright (C) 2008-2018" ) );
    gtk_about_dialog_set_comments ( (GtkAboutDialog*)about_dlg, _( "Based on the monitor configuration tool for LXDE" ) );
...
static void set_xrandr_info()
{
    ...
    if (g_spawn_command_line_sync( cmd->str, NULL, NULL, NULL, NULL ))
    {
        ...
        /* There is a race condition problem with g_spawn command.*/
        g_usleep(2000000);
